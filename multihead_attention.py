...
class MultiHeadScaledDotProductAttention(Layer):
    def __init__(self, units, num_heads=4, dropout_rate=0.1):
        ...
        # Separate Dense layers for Q, K, V per head
        self.Wq = [Dense(self.head_dim) for _ in range(num_heads)]
        self.Wk = [Dense(self.head_dim) for _ in range(num_heads)]
        self.Wv = [Dense(self.head_dim) for _ in range(num_heads)]

        self.final_proj = Dense(units)  # Project concatenated heads back to units
        self.dropout = Dropout(dropout_rate)

    def call(self, encoder_output, decoder_hidden):
        decoder_hidden_expanded = tf.expand_dims(decoder_hidden, 1)  # (batch, 1, hidden_dim)

        head_outputs = []
        for i in range(self.num_heads):
            # Project to queries, keys, and values
            Q = self.Wq[i](decoder_hidden_expanded)  # (batch, 1, head_dim)
            K = self.Wk[i](encoder_output)           # (batch, seq_len, head_dim)
            V = self.Wv[i](encoder_output)           # (batch, seq_len, head_dim)

            # Scaled dot-product attention
            score = tf.matmul(Q, K, transpose_b=True)  # (batch, 1, seq_len)
            score = score / tf.math.sqrt(tf.cast(self.head_dim, tf.float32))
            attention_weights = tf.nn.softmax(score, axis=-1)
            attention_weights = self.dropout(attention_weights)

            # Compute context vector
            context_vector = tf.matmul(attention_weights, V)  # (batch, 1, head_dim)
            context_vector = tf.squeeze(context_vector, axis=1)  # (batch, head_dim)
            head_outputs.append(context_vector)

        # Concatenate all heads
        concat_context = tf.concat(head_outputs, axis=-1)  # (batch, units)
        final_context = self.final_proj(concat_context)
...
class AttentionDecoder(Layer):
    def __init__(self, hidden_units, forecast_steps, dropout_rate=0.1):
        ...
        self.attention = MultiHeadScaledDotProductAttention(hidden_units, num_heads=4, dropout_rate=dropout_rate)
