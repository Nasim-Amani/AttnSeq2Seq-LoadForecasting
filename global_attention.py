...
class FlexibleLuongAttention(Layer):
    def __init__(self, units, mode="dot", dropout_rate=0.1):
        super(FlexibleLuongAttention, self).__init__()
        self.units = units
        self.mode = mode
    ...
    def call(self, encoder_output, decoder_hidden):
        ...
        if self.mode == "dot":
            # No projection, direct dot product
            score = tf.matmul(encoder_output, decoder_hidden_expanded, transpose_b=True)
        elif self.mode == "general":
            projected_decoder = self.W(decoder_hidden_expanded)
            score = tf.matmul(encoder_output, projected_decoder, transpose_b=True)
        elif self.mode == "concat":
            projected_encoder = self.W1(encoder_output)
            projected_decoder = self.W2(decoder_hidden_expanded)
            combined = tf.nn.tanh(projected_encoder + projected_decoder)
        ...
        context_vector = tf.reduce_sum(attention_weights_expanded * encoder_output, axis=1)
...
class AttentionDecoder(Layer):
    def __init__(self, hidden_units, forecast_steps, dropout_rate=0.1):
        ...
        self.attention = FlexibleLuongAttention(hidden_units)
